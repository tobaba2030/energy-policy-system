...
// 图表1：源网荷储一体化架构图
const flowDiagramPayload = JSON.stringify({
...
// 图表2：新型电力系统特征雷达图
const radarChartPayload = JSON.stringify({
...
// 图表3：计量技术发展趋势折线图
const lineChartPayload = JSON.stringify({
...
// 图表4：计量技术架构组织图
const orgChartPayload = JSON.stringify({
...
// 执行生成图表的函数
function generateChart(payload, chartName) {
  return new Promise((resolve, reject) => {
    const command = `node "${SKILL_PATH}" '${payload}'`;
    
    exec(command, { cwd: path.dirname(SKILL_PATH) }, (error, stdout, stderr) => {
      if (error) {
        console.error(`生成${chartName}失败:`, error);
        reject(error);
        return;
      }
      console.log(`${chartName}生成成功:`, stdout);
      resolve(stdout);
    });
  });
}
...
async function generateAllCharts() {
  try {
    ...
    await generateChart(flowDiagramPayload, '源网荷储一体化架构图');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(radarChartPayload, '新型电力系统特征雷达图');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(lineChartPayload, '计量技术发展趋势图');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(orgChartPayload, '计量技术架构图');
