...
// 执行生成图表的函数
function generateChart(spec, chartName, filename) {
  return new Promise((resolve, reject) => {
    // 将spec写入临时JSON文件
    const tempFile = path.join(__dirname, `${filename}.json`);
    fs.writeFileSync(tempFile, JSON.stringify(spec, null, 2));
    
    const command = `node "${SKILL_PATH}" "${tempFile}"`;
    ...
    exec(command, (error, stdout, stderr) => {
      ...
      // 保存URL到文件
      const urlFile = path.join(__dirname, 'charts', `${filename}_url.txt`);
...
async function generateAllCharts() {
  try {
    ...
    await generateChart(flowDiagramSpec, '源网荷储一体化架构图', 'flow_diagram');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(radarChartSpec, '新型电力系统特征雷达图', 'radar_chart');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(lineChartSpec, '计量技术发展趋势图', 'line_chart');
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    await generateChart(orgChartSpec, '计量技术架构图', 'org_chart');
