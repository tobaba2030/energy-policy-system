# -*- coding: utf-8 -*-
"""VPP v3 - 业务场景驱动版本，所有中文引号通过chr()运行时生成"""
import os, sys
...
SEC1_1_1 = [
    ...
    f"随着{LQ}双碳{RQ}战略深入推进，截至2025年底全国风光装机已突破14亿千瓦，新能源发电量占比超20%。"
    f"新能源的高波动性与间歇性使电力系统灵活调节能力面临巨大挑战，"
    f"虚拟电厂（Virtual Power Plant, VPP）作为聚合分布式能源资源、参与电网协同调控的新型市场主体，"
    f"已被纳入国家新型电力系统建设重点方向。"
    f"2025年，国家能源局正式启动{LQ}人工智能+{RQ}能源高价值场景建设，将虚拟电厂明确列为重点培育场景；"
    f"国家发改委、国家能源局联合印发指导意见，"
    f"明确提出{LQ}到2030年虚拟电厂调节能力达到5000万千瓦以上{RQ}的发展目标。",

    f"贵州省作为国家{LQ}西电东送{RQ}工程的重要能源枢纽，年外送电量超700亿千瓦时，"
    f"全省水电可开发量约2300万千瓦、光伏装机超2000万千瓦，"
    f"同时拥有电解铝、磷化工、铁合金、数据中心等高载能产业园区，"
    f"具备构建大规模虚拟电厂的独特资源禀赋，聚合潜力超500万千瓦。"
    f"贵州省{LQ}十四五{RQ}能源发展规划已将虚拟电厂纳入新型电力系统建设重点工程，"
    f"省发改委印发虚拟电厂建设管理工作方案（试行），"
    f"为虚拟电厂的市场化运营提供了基础制度框架。"
    f"此外，贵州作为全国首个国家大数据综合试验区，"
    f"在数据中心算力基础设施和数据治理能力方面具有先发优势，为本项目提供了良好的数字化底座。",

    f"从人工智能赋能趋势看，电力系统正从{LQ}经验调度{RQ}向{LQ}数据+AI双驱动{RQ}转型。"
    f"大语言模型、多智能体强化学习、时序预测大模型等技术的快速成熟，"
    f"为虚拟电厂在大规模资源聚合、多市场协同报价、海量约束实时优化等核心业务环节提供了全新解决方案。"
    f"然而，当前虚拟电厂在贵州及南方区域的落地应用仍以试点示范为主，"
    f"尚未形成规模化、常态化的商业运营闭环，亟需通过本项目打通{LQ}技术可行{AR}商业可用{AR}规模可复制{RQ}的完整链路。",
]
...
SEC1_2 = [
    ...
    f"在电力供需紧张时期，这些产业因刚性限电面临巨大经济损失（2023年电力紧张期间，"
    f"仅电炉企业日均影响产量约0.9万吨），亟需通过虚拟电厂的柔性调控手段实现{LQ}削峰不减产{RQ}。",

    f"本项目的实施具有四方面重要意义："
    f"一是服务贵州电力保供大局，通过虚拟电厂聚合高载能工业园区可调资源，"
    f"在供需紧张时段提供分钟级柔性调节能力，避免刚性拉闸限电，保障经济民生；"
    f"二是助力贵州清洁能源消纳，通过虚拟电厂引导负荷侧资源跟踪新能源出力曲线，"
    f"在新能源大发时段主动提升用电负荷，减少弃风弃光；"
    f"三是培育贵州电力市场新型经营主体，通过本项目构建{LQ}资源聚合-交易辅助-调控执行-结算分配{RQ}"
    f"全链路商业运营模式，形成可复制推广的虚拟电厂市场化运营标准范式；"
...
SEC3_INNOVATIONS = [
    (f"创新点一：{LQ}资源普查-潜力评估-分级准入{RQ}全流程标准化方法",
     ...
     f"支撑贵州建成首个省级全域可调资源数据库。",

     f"【与现有做法对比】"
     f"当前虚拟电厂入市以{LQ}自愿申报+人工审核{RQ}为主，运营商各自接入手册标准不统一，"
     f"难以实现跨园区、跨地市的资源聚合。本项目提出的标准化普查方法，"
     f"首次实现贵州全省统一的分级分类资源评估与准入体系。"),
]
...
def build():
    ...
    for i, hd in enumerate(['项目', '内容']):
        ...
        cell._tc.get_or_add_tcPr().append(shading)
        r.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)
    bt.rows[0].cells[0].width = Cm(4)
    bt.rows[0].cells[1].width = Cm(12)
    for row_data in SEC4_BUDGET:
        ...
        for j, txt in enumerate(row_data):
            ...
            if j == 0:
                ...
                shading = parse_xml(f'<w:shd {nsdecls("w")} w:fill="E8EAF6" w:val="clear"/>')
                cell._tc.get_or_add_tcPr().append(shading)
        tr.cells[0].width = Cm(4); tr.cells[1].width = Cm(12)
    add_br(doc)
    print('第四章完成')
